...
header_list=['#chr', 'pos1', 'pos2', 'total_cn','major_cn', 'minor_cn', 'broadAMP','dist','early_SNV','early_SNV_range','early_SNVrate/Mb', 'early_SNVrate_CI','early_SNV_var', 'clonal_SNV']
...
def count_earlySNV(chr1, pos1, pos2):
	...
	return [early, learly, hearly, earlyV, clonal]
...
while seqz_line:
	if seqz_line[0]=='#':
		...
		if MCN >= 2:
			...
			info_list=seqz_indi[0:7]+[str(dist),str(earlyn),str(learlyn)+':'+str(hearlyn),str(earlyr),str(learlyr)+':'+str(hearlyr),str(earlyvar), str(tcln), str(clonaln) ]

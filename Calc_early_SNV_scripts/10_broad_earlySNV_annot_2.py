...
header_list=['#broad_amp','dist','early_SNV','early_SNV_range', 'early_SNVrate/Mb', 'early_SNVrate_range', 'early_SNV_var', 'clonal_SNV','clonal_SNV_rate']
...
for [chr1,idx,amp] in broad_list:
	dist_sum=0; earlyn_sum=0;earlyV_sum=0;cln_sum=0
	seg_file.seek(0)
	seg_line=seg_file.readline().strip()
	while seg_line:
		if seg_line[0]=='#':
			...
			if broadinfo==amp and majCN>=2:
				dist=int(seg_indi[7]);earlyn=float(seg_indi[8]); earlyV=float(seg_indi[12]);cln=float(seg_indi[13])
				dist_sum += dist; 
				cln_sum += cln
				if minCN <= majCN*0.75:
	...
	clr=round(cln_sum*1000000/float(dist_sum),4)
	...
	info_list=[amp,str(dist_sum), str(earlyn_sum), str(learlyn_sum)+':'+str(hearlyn_sum),str(earlyr), str(learlyr)+':'+str(hearlyr),str(earlyV_sum), str(cln_sum), str(clr)]
